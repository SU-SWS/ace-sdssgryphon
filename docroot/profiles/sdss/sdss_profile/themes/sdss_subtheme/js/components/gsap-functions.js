        ...
        (context) => {
            ...
            if (backgroundSections) {
                ...
                for (const section of backgroundSections) {
                    ...
                    const mainRegion = section.closest('.main-region');
                    const isFirstInMain = mainRegion && Array.from(mainRegion.children).find(child => !child.classList.contains('jumpstart-ui-skipnav-main-anchor'))?.contains(section);
                    const paragraphItem = section.closest('.paragraph-item');
                    const stanfordPage = section.closest('.stanford-page');
                    const isFirstInStanfordPage = stanfordPage && stanfordPage.firstElementChild === paragraphItem;
                    const topNegativeMargin = (isFirstInMain && isFirstInStanfordPage) && document.querySelector('.contextual-region.basic-page');
